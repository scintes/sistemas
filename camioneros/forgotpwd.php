					...
					if ($bValidEmail) {
						...
						if (EW_ENCRYPTED_PASSWORD) {
							if (strtolower($this->Action) == "confirm") {
								...
								$rsnew = array('contrasenia' => $sPassword); // Reset the password
								$this->Update($rsnew);
							}
						} else {
...
<?php } ?>
	<div class="form-group">
		...
		<div class="col-sm-10"><input type="text" name="email" id="email" class="form-control ewControl" value="<?php ew_HtmlEncode($forgotpwd->Email) ?>" size="30" maxlength="250" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("UserEmail")) ?>"></div>
	</div>
	<div class="form-group">
