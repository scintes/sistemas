...
class clocalidades extends cTable {
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `provincia` FROM `provincias` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`codigo` = `localidades`.`id_provincia` LIMIT 1) AS `EV__id_provincia` FROM `localidades`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if ($this->id_provincia->AdvancedSearch->SearchValue <> "" ||
			$this->id_provincia->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->id_provincia->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->id_provincia->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("localidadesview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("localidadesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "localidadesadd.php?" . $this->UrlParm($parm);
		else
			return "localidadesadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("localidadesedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("localidadesadd.php", $this->UrlParm($parm));
	}
