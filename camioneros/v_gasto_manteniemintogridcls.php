...
$v_gasto_mantenieminto_grid = NULL; // Initialize page object first

class cv_gasto_mantenieminto_grid extends cv_gasto_mantenieminto {
	...
	// Page object name
	var $PageObjName = 'v_gasto_mantenieminto_grid';

	// Grid form hidden field names
	var $FormName = 'fv_gasto_manteniemintogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["v_gasto_mantenieminto"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_id_tipo_gasto") && $objForm->HasValue("o_id_tipo_gasto") && $this->id_tipo_gasto->CurrentValue <> $this->id_tipo_gasto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_hoja_mantenimiento") && $objForm->HasValue("o_id_hoja_mantenimiento") && $this->id_hoja_mantenimiento->CurrentValue <> $this->id_hoja_mantenimiento->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		...
		$this->id_hoja_mantenimiento->CurrentValue = NULL;
	...
	function LoadFormValues() {
		...
		if (!$this->id_hoja_mantenimiento->FldIsDetailKey) {
			$this->id_hoja_mantenimiento->setFormValue($objForm->GetValue("x_id_hoja_mantenimiento"));
		}
		$this->id_hoja_mantenimiento->setOldValue($objForm->GetValue("o_id_hoja_mantenimiento"));
	}
	...
	function RestoreFormValues() {
		...
		$this->id_hoja_mantenimiento->CurrentValue = $this->id_hoja_mantenimiento->FormValue;
	}
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->id_tipo_gasto->FormValue)) {
			ew_AddMessage($gsFormError, $this->id_tipo_gasto->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_hoja_mantenimiento
			$this->id_hoja_mantenimiento->SetDbValueDef($rsnew, $this->id_hoja_mantenimiento->CurrentValue, NULL, $this->id_hoja_mantenimiento->ReadOnly);

			// Check referential integrity for master table 'hoja_mantenimientos'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_hoja_mantenimientos();
			$KeyValue = isset($rsnew['id_hoja_mantenimiento']) ? $rsnew['id_hoja_mantenimiento'] : $rsold['id_hoja_mantenimiento'];
			if (strval($KeyValue) <> "") {
			...
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "hoja_mantenimientos", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		$bValidMasterRecord = TRUE;
		$sMasterFilter = $this->SqlMasterFilter_hoja_mantenimientos();
		if (strval($this->id_hoja_mantenimiento->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@codigo@", ew_AdjustSql($this->id_hoja_mantenimiento->CurrentValue), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "hoja_mantenimientos") {
			$this->id_hoja_mantenimiento->Visible = FALSE;
			if ($GLOBALS["hoja_mantenimientos"]->EventCancelled) $this->EventCancelled = TRUE;
		}
