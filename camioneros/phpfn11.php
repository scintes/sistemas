...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "ccamioneros_db") {
			$file = "camionerosdb.php";
		} else {
		$fn = "%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{DFBA9E6C-101B-48AB-A301-122D5C6C603B}";
}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('codigo'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`codigo` IN (' . $sCurUserIDList . ')';
					$sSql = $UserTable->GetSQL($sFilter, '');
					if ($RsUser = $conn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('codigo'));
