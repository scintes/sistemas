...
$tipo_gastos_add = NULL; // Initialize page object first

class ctipo_gastos_add extends ctipo_gastos {
	...
	// Page object name
	var $PageObjName = 'tipo_gastos_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("tipo_gastoslist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tipo_gastosview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("gastos_mantenimientos", $DetailTblVar) && $GLOBALS["gastos_mantenimientos"]->DetailAdd) {
				$GLOBALS["gastos_mantenimientos"]->id_tipo_gasto->setSessionValue($this->codigo->CurrentValue); // Set master key
				if (!isset($GLOBALS["gastos_mantenimientos_grid"])) $GLOBALS["gastos_mantenimientos_grid"] = new cgastos_mantenimientos_grid(); // Get detail page object
				$AddRow = $GLOBALS["gastos_mantenimientos_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["gastos_mantenimientos"]->id_tipo_gasto->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("gastos", $DetailTblVar) && $GLOBALS["gastos"]->DetailAdd) {
				$GLOBALS["gastos"]->id_tipo_gasto->setSessionValue($this->codigo->CurrentValue); // Set master key
				if (!isset($GLOBALS["gastos_grid"])) $GLOBALS["gastos_grid"] = new cgastos_grid(); // Get detail page object
				$AddRow = $GLOBALS["gastos_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["gastos"]->id_tipo_gasto->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
// Create page object
if (!isset($tipo_gastos_add)) $tipo_gastos_add = new ctipo_gastos_add();

// Page init
$tipo_gastos_add->Page_Init();

// Page main
$tipo_gastos_add->Page_Main();
...
// Page object
var tipo_gastos_add = new ew_Page("tipo_gastos_add");
tipo_gastos_add.PageID = "add"; // Page ID
var EW_PAGE_ID = tipo_gastos_add.PageID; // For backward compatibility

// Form object
var ftipo_gastosadd = new ew_Form("ftipo_gastosadd");
...
ftipo_gastosadd.Form_CustomValidate = 
...
ftipo_gastosadd.ValidateRequired = true;
<?php } else { ?>
ftipo_gastosadd.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="ftipo_gastosadd" id="ftipo_gastosadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tipo_gastos_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tipo_gastos_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("gastos_mantenimientos", explode(",", $tipo_gastos->getCurrentDetailTable())) && $gastos_mantenimientos->DetailAdd) {
?>
...
<?php } ?>
<?php
	if (in_array("gastos", explode(",", $tipo_gastos->getCurrentDetailTable())) && $gastos->DetailAdd) {
?>
<?php if ($tipo_gastos->getCurrentDetailTable() <> "") { ?>
...
<?php
$tipo_gastos_add->Page_Terminate();
?>
