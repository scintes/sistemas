...
class cuserpriv extends cnivel_usuario {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["nivel_usuario"])) $GLOBALS["nivel_usuario"] = &$this;
	...
	function EditRow() {
		...
		for ($i = 0; $i < $this->TableNameCount; $i++) {
			$Sql = "SELECT * FROM " . EW_USER_LEVEL_PRIV_TABLE . " WHERE " . 
				...
				EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . " = " . $this->codigo->CurrentValue;
			$rs = $conn->Execute($Sql);
			if ($rs && !$rs->EOF) {
...
<p><?php echo $Language->Phrase("UserLevel") ?><?php echo $Security->GetUserLevelName($nivel_usuario->codigo->CurrentValue) ?>(<?php echo $nivel_usuario->codigo->CurrentValue ?>)</p>
<?php
...
<input type="hidden" name="x_codigo" id="x_codigo" value="<?php echo $nivel_usuario->codigo->CurrentValue ?>">
...
for ($i = 0; $i < $userpriv->TableNameCount; $i++) {
	$userpriv->TempPriv = $Security->GetUserLevelPrivEx($userpriv->TableList[$i][4] . $userpriv->TableList[$i][0], $nivel_usuario->codigo->CurrentValue);
