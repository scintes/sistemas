...
//
class ctipo_gastos extends cTable {
	var $codigo;
	var $tipo_gasto;
	var $clase;
	...
	function __construct() {
		...
		// codigo
		$this->codigo = new cField('tipo_gastos', 'tipo_gastos', 'x_codigo', 'codigo', '`codigo`', '`codigo`', 3, -1, FALSE, '`codigo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->codigo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codigo'] = &$this->codigo;

		// tipo_gasto
		$this->tipo_gasto = new cField('tipo_gastos', 'tipo_gastos', 'x_tipo_gasto', 'tipo_gasto', '`tipo_gasto`', '`tipo_gasto`', 200, -1, FALSE, '`tipo_gasto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['tipo_gasto'] = &$this->tipo_gasto;

		// clase
		$this->clase = new cField('tipo_gastos', 'tipo_gastos', 'x_clase', 'clase', '`clase`', '`clase`', 200, -1, FALSE, '`clase`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		if ($sDetailUrl == "") {
			$sDetailUrl = "tipo_gastoslist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tipo_gastos`";
	}
	...
	function Update(&$rs, $where = "", $rsold = NULL) {
		...
		$rscascade = array();
		if (!is_null($rsold) && (isset($rs['codigo']) && $rsold['codigo'] <> $rs['codigo'])) { // Update detail field 'id_tipo_gasto'
			$bCascadeUpdate = TRUE;
			$rscascade['id_tipo_gasto'] = $rs['codigo']; 
		}
		if ($bCascadeUpdate) {
			if (!isset($GLOBALS["gastos"])) $GLOBALS["gastos"] = new cgastos();
			$GLOBALS["gastos"]->Update($rscascade, "`id_tipo_gasto` = " . ew_QuotedValue($rsold['codigo'], EW_DATATYPE_NUMBER));
		}
	...
	function Delete(&$rs, $where = "") {
		...
		if (!isset($GLOBALS["gastos"])) $GLOBALS["gastos"] = new cgastos();
		$rscascade = array();
		$GLOBALS["gastos"]->Delete($rscascade, "`id_tipo_gasto` = " . ew_QuotedValue($rs['codigo'], EW_DATATYPE_NUMBER));
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("tipo_gastosview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("tipo_gastosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "tipo_gastosadd.php?" . $this->UrlParm($parm);
		else
			return "tipo_gastosadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("tipo_gastosedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("tipo_gastosedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("tipo_gastosadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("tipo_gastosadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tipo_gastosdelete.php", $this->UrlParm());
	}
