...
//
// Table class for tipo_cargas
//
class ctipo_cargas extends cTable {
	var $codigo;
	var $Tipo_carga;
	var $precio_base;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tipo_cargas`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("tipo_cargasview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("tipo_cargasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "tipo_cargasadd.php?" . $this->UrlParm($parm);
		else
			return "tipo_cargasadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("tipo_cargasedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("tipo_cargasadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tipo_cargasdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->precio_base->Exportable) $Doc->ExportCaption($this->precio_base);
					if ($this->porcentaje_comision->Exportable) $Doc->ExportCaption($this->porcentaje_comision);
				} else {
					if ($this->codigo->Exportable) $Doc->ExportCaption($this->codigo);
					if ($this->Tipo_carga->Exportable) $Doc->ExportCaption($this->Tipo_carga);
					if ($this->precio_base->Exportable) $Doc->ExportCaption($this->precio_base);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->precio_base->Exportable) $Doc->ExportField($this->precio_base);
						if ($this->porcentaje_comision->Exportable) $Doc->ExportField($this->porcentaje_comision);
					} else {
						if ($this->codigo->Exportable) $Doc->ExportField($this->codigo);
						if ($this->Tipo_carga->Exportable) $Doc->ExportField($this->Tipo_carga);
						if ($this->precio_base->Exportable) $Doc->ExportField($this->precio_base);
