...
<?php include_once $EW_RELATIVE_PATH . "localidadesinfo.php" ?>
...
$localidades_addopt = NULL; // Initialize page object first

class clocalidades_addopt extends clocalidades {
	...
	// Page object name
	var $PageObjName = 'localidades_addopt';
...
// Create page object
if (!isset($localidades_addopt)) $localidades_addopt = new clocalidades_addopt();

// Page init
$localidades_addopt->Page_Init();

// Page main
$localidades_addopt->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$localidades_addopt->Page_Render();
?>
<script type="text/javascript">

// Page object
var localidades_addopt = new ew_Page("localidades_addopt");
localidades_addopt.PageID = "addopt"; // Page ID
var EW_PAGE_ID = localidades_addopt.PageID; // For backward compatibility

// Form object
var flocalidadesaddopt = new ew_Form("flocalidadesaddopt");

// Validate form
flocalidadesaddopt.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($localidades->id_provincia->FldErrMsg()) ?>");
...
flocalidadesaddopt.Form_CustomValidate = 
...
flocalidadesaddopt.ValidateRequired = true;
<?php } else { ?>
flocalidadesaddopt.ValidateRequired = false; 
<?php } ?>
...
<?php
$localidades_addopt->ShowMessage();
?>
<form name="flocalidadesaddopt" id="flocalidadesaddopt" class="ewForm form-horizontal" action="localidadesaddopt.php" method="post">
<?php if ($localidades_addopt->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $localidades_addopt->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="localidades">
<input type="hidden" name="a_addopt" id="a_addopt" value="A">
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel" for="x_localidad"><?php echo $localidades->localidad->FldCaption() ?></label>
		<div class="col-sm-9">
<input type="text" data-field="x_localidad" name="x_localidad" id="x_localidad" size="30" maxlength="150" placeholder="<?php echo ew_HtmlEncode($localidades->localidad->PlaceHolder) ?>" value="<?php echo $localidades->localidad->EditValue ?>"<?php echo $localidades->localidad->EditAttributes() ?>>
	...
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel" for="x_cp"><?php echo $localidades->cp->FldCaption() ?></label>
		<div class="col-sm-9">
<input type="text" data-field="x_cp" name="x_cp" id="x_cp" size="30" placeholder="<?php echo ew_HtmlEncode($localidades->cp->PlaceHolder) ?>" value="<?php echo $localidades->cp->EditValue ?>"<?php echo $localidades->cp->EditAttributes() ?>>
	...
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel" for="x_id_provincia"><?php echo $localidades->id_provincia->FldCaption() ?></label>
		<div class="col-sm-9">
<input type="text" data-field="x_id_provincia" name="x_id_provincia" id="x_id_provincia" size="30" placeholder="<?php echo ew_HtmlEncode($localidades->id_provincia->PlaceHolder) ?>" value="<?php echo $localidades->id_provincia->EditValue ?>"<?php echo $localidades->id_provincia->EditAttributes() ?>>
</div>
	</div>
</form>
<script type="text/javascript">
flocalidadesaddopt.Init();
</script>
<script type="text/javascript">
...
<?php
$localidades_addopt->Page_Terminate();
?>
