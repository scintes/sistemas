...
//
// Table class for v_gastos_m
//
class cv_gastos_m extends cTable {
	var $codigo;
	var $fecha;
	var $detalles;
	var $iva;
	...
	function __construct() {
		...
		$this->TableVar = 'v_gastos_m';
		$this->TableName = 'v_gastos_m';
		$this->TableType = 'VIEW';
		...
		// codigo
		$this->codigo = new cField('v_gastos_m', 'v_gastos_m', 'x_codigo', 'codigo', '`codigo`', '`codigo`', 3, -1, FALSE, '`codigo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->codigo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['codigo'] = &$this->codigo;

		// fecha
		$this->fecha = new cField('v_gastos_m', 'v_gastos_m', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%d/%m/%Y\')', 133, 7, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha'] = &$this->fecha;

		// detalles
		$this->detalles = new cField('v_gastos_m', 'v_gastos_m', 'x_detalles', 'detalles', '`detalles`', '`detalles`', 200, -1, FALSE, '`detalles`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['detalles'] = &$this->detalles;

		// iva
		$this->iva = new cField('v_gastos_m', 'v_gastos_m', 'x_iva', 'iva', '`iva`', '`iva`', 131, -1, FALSE, '`iva`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->iva->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['iva'] = &$this->iva;

		// Importe
		$this->Importe = new cField('v_gastos_m', 'v_gastos_m', 'x_Importe', 'Importe', '`Importe`', '`Importe`', 131, -1, FALSE, '`Importe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Importe->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['Importe'] = &$this->Importe;

		// id_tipo_gasto
		$this->id_tipo_gasto = new cField('v_gastos_m', 'v_gastos_m', 'x_id_tipo_gasto', 'id_tipo_gasto', '`id_tipo_gasto`', '`id_tipo_gasto`', 3, -1, FALSE, '`EV__id_tipo_gasto`', TRUE, TRUE, TRUE, 'FORMATTED TEXT');
		$this->id_tipo_gasto->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_tipo_gasto'] = &$this->id_tipo_gasto;

		// id_hoja_ruta
		$this->id_hoja_ruta = new cField('v_gastos_m', 'v_gastos_m', 'x_id_hoja_ruta', 'id_hoja_ruta', '`id_hoja_ruta`', '`id_hoja_ruta`', 3, -1, FALSE, '`id_hoja_ruta`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		$this->id_hoja_mantenimiento = new cField('v_gastos_m', 'v_gastos_m', 'x_id_hoja_mantenimiento', 'id_hoja_mantenimiento', '`id_hoja_mantenimiento`', '`id_hoja_mantenimiento`', 3, -1, FALSE, '`id_hoja_mantenimiento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`v_gastos_m`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `tipo_gasto` FROM `tipo_gastos` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`codigo` = `v_gastos_m`.`id_tipo_gasto` LIMIT 1) AS `EV__id_tipo_gasto` FROM `v_gastos_m`" .
			") `EW_TMP_TABLE`";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("v_gastos_mview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("v_gastos_mview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "v_gastos_madd.php?" . $this->UrlParm($parm);
		else
			return "v_gastos_madd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("v_gastos_medit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("v_gastos_madd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("v_gastos_mdelete.php", $this->UrlParm());
	}
