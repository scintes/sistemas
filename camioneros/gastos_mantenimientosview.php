...
$gastos_mantenimientos_view = NULL; // Initialize page object first

class cgastos_mantenimientos_view extends cgastos_mantenimientos {
	...
	// Page object name
	var $PageObjName = 'gastos_mantenimientos_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "gastos_mantenimientoslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "gastos_mantenimientoslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($gastos_mantenimientos_view)) $gastos_mantenimientos_view = new cgastos_mantenimientos_view();

// Page init
$gastos_mantenimientos_view->Page_Init();

// Page main
$gastos_mantenimientos_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$gastos_mantenimientos_view->Page_Render();
?>
...
// Page object
var gastos_mantenimientos_view = new ew_Page("gastos_mantenimientos_view");
gastos_mantenimientos_view.PageID = "view"; // Page ID
var EW_PAGE_ID = gastos_mantenimientos_view.PageID; // For backward compatibility

// Form object
var fgastos_mantenimientosview = new ew_Form("fgastos_mantenimientosview");

// Form_CustomValidate event
fgastos_mantenimientosview.Form_CustomValidate = 
...
fgastos_mantenimientosview.ValidateRequired = true;
<?php } else { ?>
fgastos_mantenimientosview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgastos_mantenimientosview.Lists["x_id_tipo_gasto"] = {"LinkField":"x_codigo","Ajax":true,"AutoFill":false,"DisplayFields":["x_tipo_gasto","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php $gastos_mantenimientos_view->ExportOptions->Render("body") ?>
<?php
	foreach ($gastos_mantenimientos_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($gastos_mantenimientos->Export == "") { ?>
...
<?php } ?>
<?php $gastos_mantenimientos_view->ShowPageHeader(); ?>
<?php
$gastos_mantenimientos_view->ShowMessage();
?>
<?php if ($gastos_mantenimientos->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($gastos_mantenimientos_view->Pager)) $gastos_mantenimientos_view->Pager = new cNumericPager($gastos_mantenimientos_view->StartRec, $gastos_mantenimientos_view->DisplayRecs, $gastos_mantenimientos_view->TotalRecs, $gastos_mantenimientos_view->RecRange) ?>
<?php if ($gastos_mantenimientos_view->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($gastos_mantenimientos_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $gastos_mantenimientos_view->PageUrl() ?>start=<?php echo $gastos_mantenimientos_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($gastos_mantenimientos_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $gastos_mantenimientos_view->PageUrl() ?>start=<?php echo $gastos_mantenimientos_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($gastos_mantenimientos_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $gastos_mantenimientos_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($gastos_mantenimientos_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $gastos_mantenimientos_view->PageUrl() ?>start=<?php echo $gastos_mantenimientos_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($gastos_mantenimientos_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $gastos_mantenimientos_view->PageUrl() ?>start=<?php echo $gastos_mantenimientos_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<?php } ?>
...
<?php } ?>
<form name="fgastos_mantenimientosview" id="fgastos_mantenimientosview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($gastos_mantenimientos_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $gastos_mantenimientos_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="gastos_mantenimientos">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($gastos_mantenimientos->codigo->Visible) { // codigo ?>
	<tr id="r_codigo">
		<td><span id="elh_gastos_mantenimientos_codigo"><?php echo $gastos_mantenimientos->codigo->FldCaption() ?></span></td>
...
<?php echo $gastos_mantenimientos->codigo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($gastos_mantenimientos->detalle->Visible) { // detalle ?>
	<tr id="r_detalle">
		<td><span id="elh_gastos_mantenimientos_detalle"><?php echo $gastos_mantenimientos->detalle->FldCaption() ?></span></td>
...
<?php echo $gastos_mantenimientos->detalle->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($gastos_mantenimientos->fecha->Visible) { // fecha ?>
	<tr id="r_fecha">
		<td><span id="elh_gastos_mantenimientos_fecha"><?php echo $gastos_mantenimientos->fecha->FldCaption() ?></span></td>
...
<?php echo $gastos_mantenimientos->fecha->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($gastos_mantenimientos->id_tipo_gasto->Visible) { // id_tipo_gasto ?>
	<tr id="r_id_tipo_gasto">
		<td><span id="elh_gastos_mantenimientos_id_tipo_gasto"><?php echo $gastos_mantenimientos->id_tipo_gasto->FldCaption() ?></span></td>
...
<?php echo $gastos_mantenimientos->id_tipo_gasto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($gastos_mantenimientos->id_hoja_mantenimeinto->Visible) { // id_hoja_mantenimeinto ?>
	<tr id="r_id_hoja_mantenimeinto">
		<td><span id="elh_gastos_mantenimientos_id_hoja_mantenimeinto"><?php echo $gastos_mantenimientos->id_hoja_mantenimeinto->FldCaption() ?></span></td>
...
<script type="text/javascript">
fgastos_mantenimientosview.Init();
</script>
<?php
$gastos_mantenimientos_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($gastos_mantenimientos->Export == "") { ?>
...
<?php
$gastos_mantenimientos_view->Page_Terminate();
?>
