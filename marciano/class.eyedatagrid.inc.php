...
class EyeDataGrid
{
	...
	private $show_checkboxes = false; // Show checkboxes
        private $show_radiobutton = false; // Muestra el radiobutton
	...
	private $create_button = false; // Show create button
        private $create_button2 = false; // Show create button
	...
	private $image_path = ''; // Path to images
        private $ver_eliminados = false; // Nomuestra los registros eliminados en la grilla 
        private $valor_seleccionado = ''; // Guarda el valor seleccionado por los RadioButton
        
        // Imagenes requeridas para la grilla
	public $img_edit = 'edit.png';
	public $img_delete = 'delete.png';
        public $img_delete_virtual = 'deleteV.png';
        public $img_recuperar = 'recuperarV.png';
	public $img_create = 'create.png';
	public $img_reset = 'reset.png';
	public $img_impresion = 'imprimir.png';
	...
	const STDCTRL_DELETE_VIRTUAL = 5;
	const STDCTRL_RECUPERAR = 6;
	...
	const STDCTRL_IMPRESION = 11;
	const ORDER_DESC = 'DESC';
	const ORDER_ASC = 'ASC';

	// Testo por defecto
	const TXT_RESET = 'Reiniciar listado';
	const TXT_NORESULTS = 'No se encontro resultados!<br> Pruebe tildando realizar la busqueda con la opción de VER ELIMINADOS';
	...
	*/
    ...
    public function dar_datos_de_seleccion_de_radiobutton(){
           return $this->valor_seleccionado;
	...
	*/
	...
	public function showCheckboxes($show = true)
	{
        ...
        /**
	 * Show radio button on each row
	 *
	 * @param boolean $show
	 */
	public function showRadiobutton($show = true)
	{
		$this->show_radiobutton = $show;
	}
	...
	private function setFilter($column, $value){
		$this->filter = array('Column' => $column,'Value' => $value);                
	}
	...
	*/
	public function addStandardControl($type, $action, $action_type = self::TYPE_ONCLICK)
	{
		...
		switch ($type)
		{       // ----------------------------------------------------- 
			// Agregamos los botones en la grilla
                        // ----------------------------------------------------- 
                        case self::STDCTRL_IMPRESION:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_impresion . '" alt="Imprimir" title="Imprimir" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_EDIT:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_edit . '" alt="Edit" title="Modificar" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_DELETE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_delete . '" alt="Delete" title="Borrado físico" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_DELETE_VIRTUAL:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_delete_virtual . '" alt="DeleteV" title="Borrado virtual" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_RECUPERAR:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_recuperar . '" alt="Recobrar" title="Recobrar" class="tbl-control-image"></a>';
				break;
			default:
	...
	*/
	public function showCreateButton($action, $action_type = self::TYPE_ONCLICK, $text = 'New Record', $imagen='')
	{
               // $this->cantidad_botones = $this->cantidad_botones + 1;                
                //echo $this->cantidad_botones;
                //if ($this->cantidad_botones>12)
                //    die();
		$action = $this->parseLinkAction($action, $action_type);               
                //array($this->cantidad_botones =>
		$this->create_button = array('Action' => $action,'Text' => $text,'imagen'=>$imagen);
	}
	/**
	* Adds a second create control above the table
	*
	* @param string $action The action associated to the create (onclick code or href link)
	* @param integer $action_type The type of action, use TYPE_ONCLICK or TYPE_HREF
	* @param string $text The textual description of the create
	*/
        public function showCreateButton2($action, $action_type = self::TYPE_ONCLICK, $text = 'New Record', $imagen='')
	{
                //$this->cantidad_botones = $this->cantidad_botones + 1;                
                //echo $this->cantidad_botones;
                //if ($this->cantidad_botones>12)
		...
		$action = $this->parseLinkAction($action, $action_type);               
                //array($this->cantidad_botones =>
		$this->create_button2 = array('Action' => $action,'Text' => $text,'imagen'=>$imagen);
	}
	...
	*/
	...
	{
		$this->limit = array('Low' => $low,'High' => $high);
	}
	...
	*/
	private function buildHeader()
	{
		...
		// Show checkboxes y/o radiobutton
		if (($this->show_checkboxes)||($this->show_radiobutton)){       

                    $this->column_count++;
                    echo '<td class="tbl-header tbl-checkall">';
                    if ($this->show_checkboxes){
                         echo '<input type="checkbox" name="checkall" onclick="tblToggleCheckAll()">';
                    }
                    echo '</td>';
		}
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">Se han encontrados <em>' . $this->row_count . '</em> resultado';
	...
	*/
	public function printTable()
	{
		...
		if (!$this->isAjaxUsed())
		{
			...
			echo '<script type="text/javascript">function updateTable() {var form=jQuery(".formulario").serialize(); window.location = "?" + params+"&"+form; }';
                        echo '</script>';
		}
		...
		if ($this->create_button){                       
                                echo '<span class="tbl-create"><a '.$this->create_button['Action'].' title="'.$this->create_button['Text'].'">
                                      <img src="'.$this->image_path.$this->img_create.'" class="tbl-create-image">'.$this->create_button['Text'].'</a></span>';                            // linea modificada de la comentada
                }
                // -------------------------------------------- por sergio ----------------------------------
                if ($this->create_button2){
                                echo '<span class="tbl-create"><a '.$this->create_button2['Action'].' title="'.$this->create_button2['Text'].'">
                                      <img src="'.$this->image_path.$this->create_button2['imagen'].'" class="tbl-create-image">'.$this->create_button2['Text'].'</a></span>';                            // linea modificada de la comentada
                }
                //-------------------------------------------------------------------------------------------
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				if ($first == 0){
					$first = $line; // First line
                                        $seleccionar_radio = true;
                                }else{
                                    $seleccionar_radio = false;
                                }
				if ($this->show_row_number)
				...
				if ($this->show_checkboxes)
					echo '<td align="center"><input type="checkbox" class="tbl-checkbox" id="checkbox" name="tbl-checkbox" value="' . $row[$this->primary] . '"></td>';
                                ...
                                //-------------------------------------------------------------------
                                // Agregado de RADIOBUTTON en la tabla para la seleccion de registro
                                //-------------------------------------------------------------------
				if ($this->show_radiobutton){
                                        $row_tem = $row;     $res_tem = '';
                                        // concatenamos los datos del registro de la fila seleccionada con el "|"
                                        foreach ($row_tem as $key_temp => $value_temp){
                                            	if (in_array($key_tem, $this->hidden))
						continue;
                                                $res_tem = $res_tem.'|'.$value_temp;
                                        }
                                        if ($res_tem!='')$res_tem = $res_tem.'|';                                      
                                        echo '<td align="center"><input type="radio" checked=$seleccionar_radio class="tbl-radiobutton" id="" name="tbl-radiobox" value="' . $row[$this->primary] .'" onclick="clave_seleccionada_con_radiobutton(\''.$res_tem.'\');" ></td>';
                                }
				...
				foreach ($row as $key => $value){
					...
					if (array_key_exists($key, $this->type))
					{
						...
						switch ($type)
						{
							...
							case self::TYPE_DATE:
                                                               if ($criteria_2 == true)
									$value = date('d/m/Y', strtotime($value));//$value = date($criteria, strtotime($value));
								else
									$value = date('d/m/Y', $value);// $value = date($criteria, $value);
								break;
					...
					echo '<td class="tbl-cell">' . $value .'</td>'; // es donde escribe el dato de la tabla
				}
	...
	public static function useAjaxTable($responce = '')
	{
		...
		echo "if (xmlHttp == null) {alert('Su navegador no soporta AJAX'); }\n";
		echo "return xmlHttp; }\n";
	...
	*/  /* $('#valor_seleccionado').val(dato);*/
	public function printJavascript(){
		...
		echo "function tblReset() { params = '&page=1'; updateTable(); }\n";
                echo "function clave_seleccionada_con_radiobutton(dato) { $('#h_seleccion').val(dato);  };\n";
		echo "</script>\n";
	}
}