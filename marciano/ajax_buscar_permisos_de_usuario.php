...
FROM pantallas_items AS pi 
        INNER JOIN pantallas_permisos AS pp ON pp.id_pantalla = pi.codigo
        INNER JOIN pantallas_secciones AS ps ON pi.id_seccion = ps.codigo AND pp.id_seccion = ps.codigo
WHERE pi.activo = 'S' AND  ps.Activo = 'S' AND pp.id_usuario = ".$id." 
ORDER BY pi.etiqueta ASC";    
 
 */
$sql = "SELECT pi.codigo, pi.etiqueta, pi.icono, pi.hint, ps.nombre AS seccion, CONCAT(pi.id_seccion,'-',pi.codigo) AS idsp
FROM pantallas_permisos AS pp 
        INNER JOIN  pantallas_items AS pi ON pp.id_pantalla = pi.codigo
        INNER JOIN pantallas_secciones AS ps ON pp.id_seccion = ps.codigo
WHERE pi.activo = 'S' AND  ps.Activo = 'S' AND pp.id_usuario = ".$id."  
ORDER BY pi.etiqueta ASC";
