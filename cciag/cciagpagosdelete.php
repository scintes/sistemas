...
$pagos_delete = NULL; // Initialize page object first

class cpagos_delete extends cpagos {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("cciagpagoslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in pagos class, pagosinfo.php
...
// Create page object
if (!isset($pagos_delete)) $pagos_delete = new cpagos_delete();

// Page init
$pagos_delete->Page_Init();

// Page main
$pagos_delete->Page_Main();
...
// Page object
var pagos_delete = new ew_Page("pagos_delete");
pagos_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = pagos_delete.PageID; // For backward compatibility

// Form object
var fpagosdelete = new ew_Form("fpagosdelete");

// Form_CustomValidate event
fpagosdelete.Form_CustomValidate = 
...
fpagosdelete.ValidateRequired = true;
<?php } else { ?>
fpagosdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpagosdelete.Lists["x_id_deuda"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_mes","x_anio","x_monto",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpagosdelete.Lists["x_id_usuario"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_usuario","x_nombre","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($pagos_delete->Recordset = $pagos_delete->LoadRecordset())
	$pagos_deleteTotalRecs = $pagos_delete->Recordset->RecordCount(); // Get record count
if ($pagos_deleteTotalRecs <= 0) { // No record found, exit
	if ($pagos_delete->Recordset)
		$pagos_delete->Recordset->Close();
	$pagos_delete->Page_Terminate("cciagpagoslist.php"); // Return to list
}
?>
...
?>
<form name="fpagosdelete" id="fpagosdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<table id="tbl_pagosdelete" class="ewTable ewTableSeparate">
<?php echo $pagos->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($pagos->id->Visible) { // id ?>
		<td><span id="elh_pagos_id" class="pagos_id"><?php echo $pagos->id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($pagos->id_deuda->Visible) { // id_deuda ?>
		<td><span id="elh_pagos_id_deuda" class="pagos_id_deuda"><?php echo $pagos->id_deuda->FldCaption() ?></span></td>
<?php } ?>
<?php if ($pagos->fecha->Visible) { // fecha ?>
		<td><span id="elh_pagos_fecha" class="pagos_fecha"><?php echo $pagos->fecha->FldCaption() ?></span></td>
<?php } ?>
<?php if ($pagos->monto->Visible) { // monto ?>
		<td><span id="elh_pagos_monto" class="pagos_monto"><?php echo $pagos->monto->FldCaption() ?></span></td>
<?php } ?>
<?php if ($pagos->id_usuario->Visible) { // id_usuario ?>
		<td><span id="elh_pagos_id_usuario" class="pagos_id_usuario"><?php echo $pagos->id_usuario->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$pagos_delete->RecCnt = 0;
$i = 0;
while (!$pagos_delete->Recordset->EOF) {
	...
	// Set row properties
	$pagos->ResetAttrs();
	$pagos->RowType = EW_ROWTYPE_VIEW; // View
...
<span id="el<?php echo $pagos_delete->RowCnt ?>_pagos_id" class="control-group pagos_id">
...
<span id="el<?php echo $pagos_delete->RowCnt ?>_pagos_id_deuda" class="control-group pagos_id_deuda">
...
<span id="el<?php echo $pagos_delete->RowCnt ?>_pagos_fecha" class="control-group pagos_fecha">
...
<span id="el<?php echo $pagos_delete->RowCnt ?>_pagos_monto" class="control-group pagos_monto">
...
<span id="el<?php echo $pagos_delete->RowCnt ?>_pagos_id_usuario" class="control-group pagos_id_usuario">
