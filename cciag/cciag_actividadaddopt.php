...
$actividad_addopt = NULL; // Initialize page object first

class cactividad_addopt extends cactividad {
	...
	// Page object name
	var $PageObjName = 'actividad_addopt';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow()) { // Add successful
					...
					$row["x_id_rubro"] = $this->id_rubro->DbValue;
					$row["x_actividad"] = $this->actividad->DbValue;
					$row["x_descripcion"] = $this->descripcion->DbValue;
					$row["x_activa"] = $this->activa->DbValue;
					if (!EW_DEBUG_ENABLED && ob_get_length())
						ob_end_clean();
...
// Create page object
if (!isset($actividad_addopt)) $actividad_addopt = new cactividad_addopt();

// Page init
$actividad_addopt->Page_Init();

// Page main
$actividad_addopt->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$actividad_addopt->Page_Render();
?>
<script type="text/javascript">

// Page object
var actividad_addopt = new ew_Page("actividad_addopt");
actividad_addopt.PageID = "addopt"; // Page ID
var EW_PAGE_ID = actividad_addopt.PageID; // For backward compatibility

// Form object
var factividadaddopt = new ew_Form("factividadaddopt");

// Validate form
factividadaddopt.Validate = function() {
...
factividadaddopt.Form_CustomValidate = 
...
factividadaddopt.ValidateRequired = true;
<?php } else { ?>
factividadaddopt.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
factividadaddopt.Lists["x_id_rubro"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_rubro","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php
$actividad_addopt->ShowMessage();
?>
<form name="factividadaddopt" id="factividadaddopt" class="ewForm form-horizontal" action="cciag_actividadaddopt.php" method="post">
<?php if ($actividad_addopt->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $actividad_addopt->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="actividad">
<input type="hidden" name="a_addopt" id="a_addopt" value="A">
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel" for="x_id_rubro"><?php echo $actividad->id_rubro->FldCaption() ?></label>
		<div class="col-sm-9">
...
<input type="hidden" name="s_x_id_rubro" id="s_x_id_rubro" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>&amp;f0=<?php echo ew_Encrypt("`id` = {filter_value}"); ?>&amp;t0=3">
	...
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel" for="x_actividad"><?php echo $actividad->actividad->FldCaption() ?></label>
		<div class="col-sm-9">
<input type="text" data-field="x_actividad" name="x_actividad" id="x_actividad" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($actividad->actividad->PlaceHolder) ?>" value="<?php echo $actividad->actividad->EditValue ?>"<?php echo $actividad->actividad->EditAttributes() ?>>
	...
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel" for="x_descripcion"><?php echo $actividad->descripcion->FldCaption() ?></label>
		<div class="col-sm-9">
<textarea data-field="x_descripcion" name="x_descripcion" id="x_descripcion" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($actividad->descripcion->PlaceHolder) ?>"<?php echo $actividad->descripcion->EditAttributes() ?>><?php echo $actividad->descripcion->EditValue ?></textarea>
	...
	<div class="form-group">
		<label class="col-sm-3 control-label ewLabel"><?php echo $actividad->activa->FldCaption() ?></label>
		<div class="col-sm-9">
...
<?php
$actividad_addopt->Page_Terminate();
?>
