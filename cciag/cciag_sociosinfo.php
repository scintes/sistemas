...
class csocios extends cTable {
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`actividad`,'" . ew_ValueSeparator(1, $this->id_actividad) . "',`descripcion`) FROM `actividad` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `socios`.`id_actividad` LIMIT 1) AS `EV__id_actividad` FROM `socios`" .
			") `EW_TMP_TABLE`";
