...
$montos_grid = NULL; // Initialize page object first

class cmontos_grid extends cmontos {
	...
	// Page object name
	var $PageObjName = 'montos_grid';

	// Grid form hidden field names
	var $FormName = 'fmontosgrid';
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "resumen_por_mes_de_pago") {
			global $resumen_por_mes_de_pago;
			$rsmaster = $resumen_por_mes_de_pago->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("cciag_resumen_por_mes_de_pagolist.php"); // Return to master page
			} else {
				$resumen_por_mes_de_pago->LoadListRowValues($rsmaster);
				$resumen_por_mes_de_pago->RowType = EW_ROWTYPE_MASTER; // Master row
				$resumen_por_mes_de_pago->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_importe") && $objForm->HasValue("o_importe") && $this->importe->CurrentValue <> $this->importe->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fecha_creacion") && $objForm->HasValue("o_fecha_creacion") && $this->fecha_creacion->CurrentValue <> $this->fecha_creacion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_activa") && $objForm->HasValue("o_activa") && $this->activa->CurrentValue <> $this->activa->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_usuario") && $objForm->HasValue("o_id_usuario") && $this->id_usuario->CurrentValue <> $this->id_usuario->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->id_usuario->setSessionValue("");
			}
	...
	function LoadFormValues() {
		...
		$this->fecha_creacion->setOldValue($objForm->GetValue("o_fecha_creacion"));
		...
		$this->id_usuario->setOldValue($objForm->GetValue("o_id_usuario"));
	}
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "resumen_por_mes_de_pago") {
				$this->id_usuario->CurrentValue = $this->id_usuario->getSessionValue();
			}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "resumen_por_mes_de_pago") {
			$this->id_usuario->Visible = FALSE;
			if ($GLOBALS["resumen_por_mes_de_pago"]->EventCancelled) $this->EventCancelled = TRUE;
		}
