...
class cpagos_delete extends cpagos {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("cciag_pagoslist.php"); // Prevent SQL injection, return to list
		...
		if ($this->Recordset = ew_LoadRecordset($sql)) {
			...
			if (!$res) $this->Page_Terminate("cciag_pagoslist.php"); // Return to list
		}
...
if ($pagos_deleteTotalRecs <= 0) { // No record found, exit
	...
	$pagos_delete->Page_Terminate("cciag_pagoslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
?>
<form name="fpagosdelete" id="fpagosdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pagos_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pagos_delete->Token ?>">
<?php } ?>
...
<?php if ($pagos->id->Visible) { // id ?>
		<th><span id="elh_pagos_id" class="pagos_id"><?php echo $pagos->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pagos->id_deuda->Visible) { // id_deuda ?>
		<th><span id="elh_pagos_id_deuda" class="pagos_id_deuda"><?php echo $pagos->id_deuda->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pagos->fecha->Visible) { // fecha ?>
		<th><span id="elh_pagos_fecha" class="pagos_fecha"><?php echo $pagos->fecha->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pagos->monto->Visible) { // monto ?>
		<th><span id="elh_pagos_monto" class="pagos_monto"><?php echo $pagos->monto->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
