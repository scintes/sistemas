...
class csocios_view extends csocios {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["socio_nro"] <> "") {
				...
				$sReturnUrl = "cciagsocioslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cciagsocioslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cciagsocioslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$item->Visible = ($this->EditUrl <> "" && $Security->IsLoggedIn()&& $this->ShowOptionLink('edit'));

		// Copy
		$item = &$option->Add("copy");
		$item->Body = "<a class=\"ewAction ewCopy\" href=\"" . ew_HtmlEncode($this->CopyUrl) . "\">" . $Language->Phrase("ViewPageCopyLink") . "</a>";
		$item->Visible = ($this->CopyUrl <> "" && $Security->IsLoggedIn() && $this->ShowOptionLink('add'));

		// Delete
		$item = &$option->Add("delete");
		$item->Body = "<a class=\"ewAction ewDelete\" href=\"" . ew_HtmlEncode($this->DeleteUrl) . "\">" . $Language->Phrase("ViewPageDeleteLink") . "</a>";
		$item->Visible = ($this->DeleteUrl <> "" && $Security->IsLoggedIn() && $this->ShowOptionLink('delete'));
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_socios\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_socios',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fsociosview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
<?php if (!$socios_view->ExportOptions->UseDropDownButton) { ?>
</div>
...
?>
<form name="fsociosview" id="fsociosview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="socios">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_sociosview" class="table table-bordered table-striped">
...
<div id="orig_socios_id_usuario" class="hide">
<span id="el_socios_id_usuario" class="control-group">
<span<?php echo $socios->id_usuario->ViewAttributes() ?>>
<?php echo $socios->id_usuario->ViewValue ?></span>
</span>
</div>
<div id="swfo_x_id_usuario"></div><script type="text/javascript">
ewFlashSWFs[ewFlashSWFs.length] = jQuery.extend({"id":"swfo_x_id_usuario","width":200,"height":200,"version":"9.0.0","bgcolor":"#FFFFFF"}, {
});
</script>
</td>
	</tr>
