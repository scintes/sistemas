...
$tramites_view = NULL; // Initialize page object first

class ctramites_view extends ctramites {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cciag_tramiteslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cciag_tramiteslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		if ($GLOBALS["seguimiento_tramites_grid"] && $GLOBALS["seguimiento_tramites_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'seguimiento_tramites')) {
		...
		if ($GLOBALS["seguimiento_tramites_grid"] && $GLOBALS["seguimiento_tramites_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'seguimiento_tramites')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'seguimiento_tramites');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (seguimiento_tramites)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("seguimiento_tramites", explode(",", $this->getCurrentDetailTable()))) {
			global $seguimiento_tramites;
			if (!isset($seguimiento_tramites)) $seguimiento_tramites = new cseguimiento_tramites;
			$rsdetail = $seguimiento_tramites->LoadRs($seguimiento_tramites->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$seguimiento_tramites->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("seguimiento_tramites", $DetailTblVar)) {
				...
				if ($GLOBALS["seguimiento_tramites_grid"]->DetailView) {
					$GLOBALS["seguimiento_tramites_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($tramites_view)) $tramites_view = new ctramites_view();

// Page init
$tramites_view->Page_Init();

// Page main
$tramites_view->Page_Main();
...
// Page object
var tramites_view = new ew_Page("tramites_view");
tramites_view.PageID = "view"; // Page ID
var EW_PAGE_ID = tramites_view.PageID; // For backward compatibility

// Form object
var ftramitesview = new ew_Form("ftramitesview");

// Form_CustomValidate event
ftramitesview.Form_CustomValidate = 
...
ftramitesview.ValidateRequired = true;
<?php } else { ?>
ftramitesview.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $tramites_view->ExportOptions->Render("body") ?>
<?php
	foreach ($tramites_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($tramites->Export == "") { ?>
...
<?php } ?>
<?php $tramites_view->ShowPageHeader(); ?>
<?php
$tramites_view->ShowMessage();
?>
<form name="ftramitesview" id="ftramitesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tramites_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tramites_view->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($tramites->id_usuario->Visible) { // id_usuario ?>
	<tr id="r_id_usuario">
...
$Files[$i] = str_replace("%f", ew_HtmlEncode(ew_UploadPathEx(FALSE, $tramites->archivo->UploadPath) . $Files[$i]), $tramites->archivo->ViewValue);
?>
<li>
<?php if ($tramites->archivo->LinkAttributes() <> "") { ?>
<?php if (!empty($tramites->archivo->Upload->DbValue)) { ?>
<a<?php echo $tramites->archivo->LinkAttributes() ?>><?php echo $tramites->archivo->ViewValue ?></a>
<?php } elseif (!in_array($tramites->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($tramites->archivo->Upload->DbValue)) { ?>
<?php echo $tramites->archivo->ViewValue ?>
<?php } elseif (!in_array($tramites->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php
	if (in_array("seguimiento_tramites", explode(",", $tramites->getCurrentDetailTable())) && $seguimiento_tramites->DetailView) {
?>
