...
class cmontos_list extends cmontos {
	...
	function __construct() {
		...
		$this->AddUrl = "cciag_montosadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "cciag_montosdelete.php";
		$this->MultiUpdateUrl = "cciag_montosupdate.php";
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->fecha_creacion, $Default, FALSE); // fecha_creacion
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmontoslistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"cciag_montossrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fmontoslistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_montos\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_montos',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fmontoslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->fecha_creacion); // fecha_creacion
		$this->AddSearchQueryString($sQry, $this->activa); // activa
...
<?php } ?>
<?php if ($montos_list->TotalRecs > 0 && $montos_list->ExportOptions->Visible()) { ?>
<?php $montos_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($montos_list->SearchOptions->Visible()) { ?>
<?php $montos_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($montos->Export == "") { ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		if ($montos_list->TotalRecs <= 0)
			$montos_list->TotalRecs = $montos->SelectRecordCount();
	} else {
		if (!$montos_list->Recordset && ($montos_list->Recordset = $montos_list->LoadRecordset()))
	...
	if ($montos->CurrentAction == "" && $montos_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$montos_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($montos_list->SearchWhere == "0=101")
			$montos_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$montos_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$montos_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($montos->Export == "" && $montos->CurrentAction == "") { ?>
<form name="fmontoslistsrch" id="fmontoslistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($montos_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fmontoslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="montos">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($montos_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($montos_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $montos_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($montos_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($montos_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($montos_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($montos_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
?>
<?php if ($montos_list->TotalRecs > 0 || $montos->CurrentAction <> "") { ?>
<div class="ewGrid">
<?php if ($montos->Export == "") { ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $montos_list->PageUrl() ?>start=<?php echo $montos_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($montos_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $montos_list->PageUrl() ?>start=<?php echo $montos_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $montos_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($montos_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $montos_list->PageUrl() ?>start=<?php echo $montos_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($montos_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $montos_list->PageUrl() ?>start=<?php echo $montos_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $montos_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $montos_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $montos_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $montos_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php } ?>
<form name="fmontoslist" id="fmontoslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($montos_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $montos_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="montos">
<div id="gmp_montos" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($montos_list->TotalRecs > 0) { ?>
<table id="tbl_montoslist" class="table ewTable">
...
// Header row
$montos->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($montos->id->Visible) { // id ?>
	<?php if ($montos->SortUrl($montos->id) == "") { ?>
		<th data-name="id"><div id="elh_montos_id" class="montos_id"><div class="ewTableHeaderCaption"><?php echo $montos->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $montos->SortUrl($montos->id) ?>',1);"><div id="elh_montos_id" class="montos_id">
...
<?php if ($montos->descripcion->Visible) { // descripcion ?>
	<?php if ($montos->SortUrl($montos->descripcion) == "") { ?>
		<th data-name="descripcion"><div id="elh_montos_descripcion" class="montos_descripcion"><div class="ewTableHeaderCaption"><?php echo $montos->descripcion->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="descripcion"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $montos->SortUrl($montos->descripcion) ?>',1);"><div id="elh_montos_descripcion" class="montos_descripcion">
...
<?php if ($montos->importe->Visible) { // importe ?>
	<?php if ($montos->SortUrl($montos->importe) == "") { ?>
		<th data-name="importe"><div id="elh_montos_importe" class="montos_importe"><div class="ewTableHeaderCaption"><?php echo $montos->importe->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="importe"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $montos->SortUrl($montos->importe) ?>',1);"><div id="elh_montos_importe" class="montos_importe">
...
<?php if ($montos->fecha_creacion->Visible) { // fecha_creacion ?>
	<?php if ($montos->SortUrl($montos->fecha_creacion) == "") { ?>
		<th data-name="fecha_creacion"><div id="elh_montos_fecha_creacion" class="montos_fecha_creacion"><div class="ewTableHeaderCaption"><?php echo $montos->fecha_creacion->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="fecha_creacion"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $montos->SortUrl($montos->fecha_creacion) ?>',1);"><div id="elh_montos_fecha_creacion" class="montos_fecha_creacion">
...
<?php if ($montos->activa->Visible) { // activa ?>
	<?php if ($montos->SortUrl($montos->activa) == "") { ?>
		<th data-name="activa"><div id="elh_montos_activa" class="montos_activa"><div class="ewTableHeaderCaption"><?php echo $montos->activa->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="activa"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $montos->SortUrl($montos->activa) ?>',1);"><div id="elh_montos_activa" class="montos_activa">
...
<?php if ($montos->id_usuario->Visible) { // id_usuario ?>
	<?php if ($montos->SortUrl($montos->id_usuario) == "") { ?>
		<th data-name="id_usuario"><div id="elh_montos_id_usuario" class="montos_id_usuario"><div class="ewTableHeaderCaption" style="white-space: nowrap;"><?php echo $montos->id_usuario->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id_usuario"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $montos->SortUrl($montos->id_usuario) ?>',1);"><div id="elh_montos_id_usuario" class="montos_id_usuario">
			<div class="ewTableHeaderBtn" style="white-space: nowrap;"><span class="ewTableHeaderCaption"><?php echo $montos->id_usuario->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($montos->id_usuario->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($montos->id_usuario->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($montos->id->Visible) { // id ?>
		<td data-name="id"<?php echo $montos->id->CellAttributes() ?>>
...
<?php } ?>
<?php if ($montos_list->TotalRecs == 0 && $montos->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($montos_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
