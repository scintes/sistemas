...
$seguimiento_tramites_grid = NULL; // Initialize page object first

class cseguimiento_tramites_grid extends cseguimiento_tramites {
	...
	// Page object name
	var $PageObjName = 'seguimiento_tramites_grid';

	// Grid form hidden field names
	var $FormName = 'fseguimiento_tramitesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["seguimiento_tramites"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 3) {
			if (strval($arKeys[0]) <> "")
				$this->id_tramite->CurrentValue = strval($arKeys[0]); // id_tramite
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->fecha->CurrentValue = strval($arKeys[1]); // fecha
			else
				$bValidKey = FALSE;
			if (strval($arKeys[2]) <> "")
				$this->hora->CurrentValue = strval($arKeys[2]); // hora
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "tramites") {
			...
			if ($GLOBALS["tramites"]->EventCancelled) $this->EventCancelled = TRUE;
		}
