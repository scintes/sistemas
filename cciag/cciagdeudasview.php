...
class cdeudas_view extends cdeudas {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "cciagdeudaslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cciagdeudaslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cciagdeudaslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_deudas\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_deudas',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fdeudasview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
fdeudasview.Lists["x_id_usuario"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_usuario","x_nombre","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if (!$deudas_view->ExportOptions->UseDropDownButton) { ?>
</div>
...
?>
<form name="fdeudasview" id="fdeudasview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="deudas">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_deudasview" class="table table-bordered table-striped">
...
<?php echo $deudas->id_usuario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
