...
//
// Table class for detalle_deudas
//
class cdetalle_deudas extends cTable {
	var $id_deuda;
	var $detalle;
	var $importe;
	...
	function __construct() {
		...
		$this->TableVar = 'detalle_deudas';
		$this->TableName = 'detalle_deudas';
		...
		// id_deuda
		$this->id_deuda = new cField('detalle_deudas', 'detalle_deudas', 'x_id_deuda', 'id_deuda', '`id_deuda`', '`id_deuda`', 3, -1, FALSE, '`id_deuda`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_deuda->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_deuda'] = &$this->id_deuda;

		// detalle
		$this->detalle = new cField('detalle_deudas', 'detalle_deudas', 'x_detalle', 'detalle', '`detalle`', '`detalle`', 200, -1, FALSE, '`detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['detalle'] = &$this->detalle;

		// importe
		$this->importe = new cField('detalle_deudas', 'detalle_deudas', 'x_importe', 'importe', '`importe`', '`importe`', 131, -1, FALSE, '`importe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "deudas") {
			if ($this->id_deuda->getSessionValue() <> "")
				$sMasterFilter .= "`id`=" . ew_QuotedValue($this->id_deuda->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "deudas") {
			if ($this->id_deuda->getSessionValue() <> "")
				$sDetailFilter .= "`id_deuda`=" . ew_QuotedValue($this->id_deuda->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`detalle_deudas`";
	}
	...
	function ApplyUserIDFilters($sFilter) {
		...
		if (!$this->AllowAnonymousUser() && $Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			if ($this->getCurrentMasterTable() == "deudas" || $this->getCurrentMasterTable() == "")
				$sFilter = $this->AddDetailUserIDFilter($sFilter, "deudas"); // Add detail User ID filter
		}
		return $sFilter;
	}
	...
	// Update Table
	var $UpdateTable = "`detalle_deudas`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "cciag_detalle_deudaslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("cciag_detalle_deudasview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("cciag_detalle_deudasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "cciag_detalle_deudasadd.php?" . $this->UrlParm($parm);
		else
			return "cciag_detalle_deudasadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("cciag_detalle_deudasedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("cciag_detalle_deudasadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cciag_detalle_deudasdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function AddMasterUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "deudas") {
			$sFilterWrk = $GLOBALS["deudas"]->AddUserIDFilter($sFilterWrk);
		}
		return $sFilterWrk;
	}
	...
	function AddDetailUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "deudas") {
			$mastertable = $GLOBALS["deudas"];
			if (!$mastertable->UserIDAllow()) {
				$sSubqueryWrk = $mastertable->GetUserIDSubquery($this->id_deuda, $mastertable->id);
