...
<?php include_once "cciagpagosinfo.php" ?>
...
$pagos_edit = NULL; // Initialize page object first

class cpagos_edit extends cpagos {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("cciagpagoslist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("cciagpagoslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!$GLOBALS["pagos"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["usuario"]->AddUserIDFilter($sWhereWrk);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_deuda
			$this->id_deuda->SetDbValueDef($rsnew, $this->id_deuda->CurrentValue, NULL, $this->id_deuda->ReadOnly);
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "cciagpagoslist.php", $this->TableVar);
...
// Create page object
if (!isset($pagos_edit)) $pagos_edit = new cpagos_edit();

// Page init
$pagos_edit->Page_Init();

// Page main
$pagos_edit->Page_Main();
...
// Page object
var pagos_edit = new ew_Page("pagos_edit");
pagos_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = pagos_edit.PageID; // For backward compatibility

// Form object
var fpagosedit = new ew_Form("fpagosedit");
...
fpagosedit.Form_CustomValidate = 
...
fpagosedit.ValidateRequired = true;
<?php } else { ?>
fpagosedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpagosedit.Lists["x_id_deuda"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_mes","x_anio","x_monto",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpagosedit.Lists["x_id_usuario"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_usuario","x_nombre","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fpagosedit" id="fpagosedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="pagos">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_pagosedit" class="table table-bordered table-striped">
...
<span id="el_pagos_id" class="control-group">
<span<?php echo $pagos->id->ViewAttributes() ?>>
<?php echo $pagos->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($pagos->id->CurrentValue) ?>">
<?php echo $pagos->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_pagos_id_deuda" class="control-group">
...
<?php echo $pagos->id_deuda->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_pagos_fecha" class="control-group">
<select data-field="x_fecha" id="x_fecha" name="x_fecha"<?php echo $pagos->fecha->EditAttributes() ?>>
<?php
if (is_array($pagos->fecha->EditValue)) {
	$arwrk = $pagos->fecha->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($pagos->fecha->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<?php echo $pagos->fecha->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_pagos_monto" class="control-group">
<input type="text" data-field="x_monto" name="x_monto" id="x_monto" size="30" placeholder="<?php echo $pagos->monto->PlaceHolder ?>" value="<?php echo $pagos->monto->EditValue ?>"<?php echo $pagos->monto->EditAttributes() ?>>
</span>
<?php echo $pagos->monto->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($pagos->id_usuario->Visible) { // id_usuario ?>
	<tr id="r_id_usuario">
		<td><span id="elh_pagos_id_usuario"><?php echo $pagos->id_usuario->FldCaption() ?></span></td>
		<td<?php echo $pagos->id_usuario->CellAttributes() ?>>
<span id="el_pagos_id_usuario" class="control-group">
<select data-field="x_id_usuario" id="x_id_usuario" name="x_id_usuario"<?php echo $pagos->id_usuario->EditAttributes() ?>>
<?php
if (is_array($pagos->id_usuario->EditValue)) {
	$arwrk = $pagos->id_usuario->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($pagos->id_usuario->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
<?php if ($arwrk[$rowcntwrk][2] <> "") { ?>
<?php echo ew_ValueSeparator(1,$pagos->id_usuario) ?><?php echo $arwrk[$rowcntwrk][2] ?>
<?php } ?>
</option>
<?php
	}
...
// Call Lookup selecting
$pagos->Lookup_Selecting($pagos->id_usuario, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<?php echo $pagos->id_usuario->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
