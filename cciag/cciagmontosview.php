...
class cmontos_view extends cmontos {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "cciagmontoslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cciagmontoslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cciagmontoslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_montos\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_montos',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fmontosview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "cciagmontoslist.php", $this->TableVar);
...
<?php if (!$montos_view->ExportOptions->UseDropDownButton) { ?>
</div>
...
?>
<form name="fmontosview" id="fmontosview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="montos">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_montosview" class="table table-bordered table-striped">
