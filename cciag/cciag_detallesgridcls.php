...
$detalles_grid = NULL; // Initialize page object first

class cdetalles_grid extends cdetalles {
	...
	// Page object name
	var $PageObjName = 'detalles_grid';

	// Grid form hidden field names
	var $FormName = 'fdetallesgrid';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["detalles"])) {
			$GLOBALS["detalles"] = &$this;

//			$GLOBALS["MasterTable"] = &$GLOBALS["Table"];
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalles"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "socios_detalles") {
			global $socios_detalles;
			$rsmaster = $socios_detalles->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("cciag_socios_detalleslist.php"); // Return to master page
			} else {
				$socios_detalles->LoadListRowValues($rsmaster);
				$socios_detalles->RowType = EW_ROWTYPE_MASTER; // Master row
				$socios_detalles->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Check referential integrity for master table 'socios_detalles'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_socios_detalles();
			$KeyValue = isset($rsnew['codigo']) ? $rsnew['codigo'] : $rsold['codigo'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@id_socio@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
				$bValidMasterRecord = FALSE;
			}
			if ($bValidMasterRecord) {
				$rsmaster = $GLOBALS["socios_detalles"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "socios_detalles", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "socios_detalles") {
		...
		if (strval($this->codigo->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@id_socio@", ew_AdjustSql($this->codigo->CurrentValue), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "socios_detalles") {
			$this->codigo->Visible = FALSE;
			if ($GLOBALS["socios_detalles"]->EventCancelled) $this->EventCancelled = TRUE;
		}
