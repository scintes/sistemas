...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "ccciag2dsauceviejo_db") {
			$file = "cciag_cciag2dsauceviejodb.php";
		} else {
		$fn = "cciag_%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{E85D8E60-21B0-46D8-A725-BE5A2EF61FC0}";
}
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setSessionParentUserID($rs->fields('id')); // Load parent User ID
...
// Include shared code
include_once "cciag_ewshared11.php";
?>
