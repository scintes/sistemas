...
class cuserlevelpermissions_search extends cuserlevelpermissions {
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "cciag_userlevelpermissionslist.php" . "?" . $sSrchStr;
						if ($this->IsModal) {
							$row = array();
							$row["url"] = $sSrchStr;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (trim(strval($this->userlevelid->AdvancedSearch->SearchValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`userlevelid`" . ew_SearchString("=", $this->userlevelid->AdvancedSearch->SearchValue, EW_DATATYPE_NUMBER);
			}
...
// Form object
var fuserlevelpermissionssearch = new ew_Form("fuserlevelpermissionssearch");
...
fuserlevelpermissionssearch.Lists["x_userlevelid"] = {"LinkField":"x_userlevelid","Ajax":true,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fuserlevelpermissionssearch" id="fuserlevelpermissionssearch" class="form-horizontal ewForm ewSearchForm" action="<?php echo ew_CurrentPage() ?>" method="post">
