...
class cpagos_edit extends cpagos {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("cciag_pagoslist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!$GLOBALS["pagos"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["deudas"]->AddUserIDFilter($sWhereWrk);
...
?>
<form name="fpagosedit" id="fpagosedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pagos_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pagos_edit->Token ?>">
<?php } ?>
...
<?php if ($pagos->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_pagos_id" class="col-sm-2 control-label ewLabel"><?php echo $pagos->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $pagos->id->CellAttributes() ?>>
<span id="el_pagos_id">
...
<?php echo $pagos->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fpagosedit", "x_fecha", "%d/%m/%Y");
</script>
<?php } ?>
</span>
