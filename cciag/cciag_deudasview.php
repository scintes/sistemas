...
class cdeudas_view extends cdeudas {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "cciag_deudaslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cciag_deudaslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cciag_deudaslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("detalle_deudas", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["detalle_deudas_grid"] && $GLOBALS["detalle_deudas_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'detalle_deudas')) {
		...
		if ($GLOBALS["detalle_deudas_grid"] && $GLOBALS["detalle_deudas_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'detalle_deudas')) {
		...
		if ($GLOBALS["detalle_deudas_grid"] && $GLOBALS["detalle_deudas_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'detalle_deudas')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'detalle_deudas');
		if ($item->Visible) {
		...
		$links = "";
		if ($GLOBALS["pagos_grid"] && $GLOBALS["pagos_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'pagos')) {
		...
		if ($GLOBALS["pagos_grid"] && $GLOBALS["pagos_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'pagos')) {
		...
		if ($GLOBALS["pagos_grid"] && $GLOBALS["pagos_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'pagos')) {
	...
	function ExportData() {
		...
		if (EW_EXPORT_DETAIL_RECORDS && in_array("detalle_deudas", explode(",", $this->getCurrentDetailTable()))) {
			...
			if (!isset($detalle_deudas)) $detalle_deudas = new cdetalle_deudas;
			$rsdetail = $detalle_deudas->LoadRs($detalle_deudas->GetDetailFilter()); // Load detail records
		...
		if (EW_EXPORT_DETAIL_RECORDS && in_array("pagos", explode(",", $this->getCurrentDetailTable()))) {
			global $pagos;
			if (!isset($pagos)) $pagos = new cpagos;
			$rsdetail = $pagos->LoadRs($pagos->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("detalle_deudas", $DetailTblVar)) {
				...
				if ($GLOBALS["detalle_deudas_grid"]->DetailView) {
					$GLOBALS["detalle_deudas_grid"]->CurrentMode = "view";
			...
			if (in_array("pagos", $DetailTblVar)) {
				...
				if ($GLOBALS["pagos_grid"]->DetailView) {
					$GLOBALS["pagos_grid"]->CurrentMode = "view";
...
// Page object
var deudas_view = new ew_Page("deudas_view");
deudas_view.PageID = "view"; // Page ID
var EW_PAGE_ID = deudas_view.PageID; // For backward compatibility

// Form object
var fdeudasview = new ew_Form("fdeudasview");
...
fdeudasview.ValidateRequired = true;
<?php } else { ?>
fdeudasview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdeudasview.Lists["x_id_socio"] = {"LinkField":"x_socio_nro","Ajax":true,"AutoFill":false,"DisplayFields":["x_propietario","x_comercio","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if ($deudas->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_deudas_id"><?php echo $deudas->id->FldCaption() ?></span></td>
...
<?php echo $deudas->mes->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $deudas->anio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $deudas->fecha->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $deudas->monto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($deudas->id_socio->Visible) { // id_socio ?>
	<tr id="r_id_socio">
		<td><span id="elh_deudas_id_socio"><?php echo $deudas->id_socio->FldCaption() ?></span></td>
		<td<?php echo $deudas->id_socio->CellAttributes() ?>>
<span id="el_deudas_id_socio">
<span<?php echo $deudas->id_socio->ViewAttributes() ?>>
<?php echo $deudas->id_socio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("detalle_deudas", explode(",", $deudas->getCurrentDetailTable())) && $detalle_deudas->DetailView) {
?>
...
<?php } ?>
<?php
	if (in_array("pagos", explode(",", $deudas->getCurrentDetailTable())) && $pagos->DetailView) {
?>
<?php if ($deudas->getCurrentDetailTable() <> "") { ?>
