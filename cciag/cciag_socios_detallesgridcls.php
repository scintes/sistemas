...
$socios_detalles_grid = NULL; // Initialize page object first

class csocios_detalles_grid extends csocios_detalles {
	...
	// Page object name
	var $PageObjName = 'socios_detalles_grid';

	// Grid form hidden field names
	var $FormName = 'fsocios_detallesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["socios_detalles"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_id_socio") && $objForm->HasValue("o_id_socio") && $this->id_socio->CurrentValue <> $this->id_socio->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_detalles") && $objForm->HasValue("o_id_detalles") && $this->id_detalles->CurrentValue <> $this->id_detalles->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fecha_alta") && $objForm->HasValue("o_fecha_alta") && $this->fecha_alta->CurrentValue <> $this->fecha_alta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fecha_baja") && $objForm->HasValue("o_fecha_baja") && $this->fecha_baja->CurrentValue <> $this->fecha_baja->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadDefaultValues() {
		...
		$this->id_detalles->CurrentValue = NULL;
		$this->id_detalles->OldValue = $this->id_detalles->CurrentValue;
		$this->fecha_alta->CurrentValue = date("d/m/Y");
	...
	function LoadFormValues() {
		...
		if (!$this->id_detalles->FldIsDetailKey) {
			$this->id_detalles->setFormValue($objForm->GetValue("x_id_detalles"));
		}
		$this->id_detalles->setOldValue($objForm->GetValue("o_id_detalles"));
		if (!$this->fecha_alta->FldIsDetailKey) {
			$this->fecha_alta->setFormValue($objForm->GetValue("x_fecha_alta"));
			$this->fecha_alta->CurrentValue = ew_UnFormatDateTime($this->fecha_alta->CurrentValue, 7);
		}
		$this->fecha_alta->setOldValue($objForm->GetValue("o_fecha_alta"));
		if (!$this->fecha_baja->FldIsDetailKey) {
			$this->fecha_baja->setFormValue($objForm->GetValue("x_fecha_baja"));
			$this->fecha_baja->CurrentValue = ew_UnFormatDateTime($this->fecha_baja->CurrentValue, 7);
		}
		$this->fecha_baja->setOldValue($objForm->GetValue("o_fecha_baja"));
	}
	...
	function RestoreFormValues() {
		...
		$this->id_detalles->CurrentValue = $this->id_detalles->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!$GLOBALS["socios_detalles"]->UserIDAllow("gridcls")) $sWhereWrk = $GLOBALS["socios"]->AddUserIDFilter($sWhereWrk);
			...
			if (trim(strval($this->id_detalles->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`codigo`" . ew_SearchString("=", $this->id_detalles->CurrentValue, EW_DATATYPE_NUMBER);
			}
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->fecha_alta->FldIsDetailKey && !is_null($this->fecha_alta->FormValue) && $this->fecha_alta->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->fecha_alta->FldCaption(), $this->fecha_alta->ReqErrMsg));
		}
		if (!ew_CheckEuroDate($this->fecha_alta->FormValue)) {
			ew_AddMessage($gsFormError, $this->fecha_alta->FldErrMsg());
		}
		if (!ew_CheckEuroDate($this->fecha_baja->FormValue)) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_detalles
			$this->id_detalles->SetDbValueDef($rsnew, $this->id_detalles->CurrentValue, NULL, $this->id_detalles->ReadOnly);

			// fecha_alta
			$this->fecha_alta->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_alta->CurrentValue, 7), NULL, $this->fecha_alta->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$sMasterFilter = $this->SqlMasterFilter_detalles();
			if (strval($this->id_detalles->CurrentValue) <> "" &&
				$this->getCurrentMasterTable() == "detalles") {
				$sMasterFilter = str_replace("@codigo@", ew_AdjustSql($this->id_detalles->CurrentValue), $sMasterFilter);
			} else {
				$sMasterFilter = "";
			}
			if ($sMasterFilter <> "") {
				$rsmaster = $GLOBALS["detalles"]->LoadRs($sMasterFilter);
				$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
				if (!$this->MasterRecordExists) {
		...
		// id_detalles
		$this->id_detalles->SetDbValueDef($rsnew, $this->id_detalles->CurrentValue, NULL, FALSE);

		// fecha_alta
		$this->fecha_alta->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_alta->CurrentValue, 7), NULL, FALSE);

		// fecha_baja
		$this->fecha_baja->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_baja->CurrentValue, 7), NULL, FALSE);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "detalles") {
			$this->id_detalles->Visible = FALSE;
			if ($GLOBALS["detalles"]->EventCancelled) $this->EventCancelled = TRUE;
		}
