...
$actividad_search = NULL; // Initialize page object first

class cactividad_search extends cactividad {
	...
	// Page object name
	var $PageObjName = 'actividad_search';
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "cciag_actividadlist.php" . "?" . $sSrchStr;
						if ($this->IsModal) {
							$row = array();
							$row["url"] = $sSrchStr;
	...
	function BuildAdvancedSearch() {
		...
		$this->BuildSearchUrl($sSrchUrl, $this->id_rubro); // id_rubro
		$this->BuildSearchUrl($sSrchUrl, $this->actividad); // actividad
	...
	function LoadSearchValues() {
		...
		// id_rubro
		$this->id_rubro->AdvancedSearch->SearchValue = ew_StripSlashes($objForm->GetValue("x_id_rubro"));
		$this->id_rubro->AdvancedSearch->SearchOperator = $objForm->GetValue("z_id_rubro");

		// actividad
		$this->actividad->AdvancedSearch->SearchValue = ew_StripSlashes($objForm->GetValue("x_actividad"));
		$this->actividad->AdvancedSearch->SearchOperator = $objForm->GetValue("z_actividad");
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (trim(strval($this->id_rubro->AdvancedSearch->SearchValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->id_rubro->AdvancedSearch->SearchValue, EW_DATATYPE_NUMBER);
			}
			...
			$this->actividad->EditValue = ew_HtmlEncode($this->actividad->AdvancedSearch->SearchValue);
	...
	function LoadAdvancedSearch() {
		...
		$this->id_rubro->AdvancedSearch->Load();
...
// Create page object
if (!isset($actividad_search)) $actividad_search = new cactividad_search();

// Page init
$actividad_search->Page_Init();

// Page main
$actividad_search->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$actividad_search->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "cciag_header.php" ?>
<script type="text/javascript">

// Page object
var actividad_search = new ew_Page("actividad_search");
actividad_search.PageID = "search"; // Page ID
var EW_PAGE_ID = actividad_search.PageID; // For backward compatibility

// Form object
var factividadsearch = new ew_Form("factividadsearch");

// Form_CustomValidate event
factividadsearch.Form_CustomValidate = 
...
factividadsearch.ValidateRequired = true;
<?php } else { ?>
factividadsearch.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
factividadsearch.Lists["x_id_rubro"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_rubro","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};

// Form object for search
// Validate function for search

factividadsearch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckInteger(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($actividad->id->FldErrMsg()) ?>");
...
<?php if (!$actividad_search->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $actividad_search->ShowPageHeader(); ?>
<?php
$actividad_search->ShowMessage();
?>
<form name="factividadsearch" id="factividadsearch" class="form-horizontal ewForm ewSearchForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($actividad_search->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $actividad_search->Token ?>">
<?php } ?>
...
<?php if ($actividad->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label for="x_id" class="<?php echo $actividad_search->SearchLabelClass ?>"><span id="elh_actividad_id"><?php echo $actividad->id->FldCaption() ?></span>	
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_id" id="z_id" value="="></p>
		</label>
		<div class="<?php echo $actividad_search->SearchRightColumnClass ?>"><div<?php echo $actividad->id->CellAttributes() ?>>
			<span id="el_actividad_id">
<input type="text" data-field="x_id" name="x_id" id="x_id" placeholder="<?php echo ew_HtmlEncode($actividad->id->PlaceHolder) ?>" value="<?php echo $actividad->id->EditValue ?>"<?php echo $actividad->id->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($actividad->id_rubro->Visible) { // id_rubro ?>
	<div id="r_id_rubro" class="form-group">
		<label for="x_id_rubro" class="<?php echo $actividad_search->SearchLabelClass ?>"><span id="elh_actividad_id_rubro"><?php echo $actividad->id_rubro->FldCaption() ?></span>	
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_id_rubro" id="z_id_rubro" value="="></p>
		</label>
		<div class="<?php echo $actividad_search->SearchRightColumnClass ?>"><div<?php echo $actividad->id_rubro->CellAttributes() ?>>
...
if (is_array($actividad->id_rubro->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($actividad->id_rubro->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php if ($actividad->actividad->Visible) { // actividad ?>
	<div id="r_actividad" class="form-group">
		<label for="x_actividad" class="<?php echo $actividad_search->SearchLabelClass ?>"><span id="elh_actividad_actividad"><?php echo $actividad->actividad->FldCaption() ?></span>	
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_actividad" id="z_actividad" value="LIKE"></p>
		</label>
		<div class="<?php echo $actividad_search->SearchRightColumnClass ?>"><div<?php echo $actividad->actividad->CellAttributes() ?>>
...
<?php if ($actividad->descripcion->Visible) { // descripcion ?>
	<div id="r_descripcion" class="form-group">
		<label for="x_descripcion" class="<?php echo $actividad_search->SearchLabelClass ?>"><span id="elh_actividad_descripcion"><?php echo $actividad->descripcion->FldCaption() ?></span>	
		...
		<div class="<?php echo $actividad_search->SearchRightColumnClass ?>"><div<?php echo $actividad->descripcion->CellAttributes() ?>>
			<span id="el_actividad_descripcion">
<input type="text" data-field="x_descripcion" name="x_descripcion" id="x_descripcion" maxlength="255" placeholder="<?php echo ew_HtmlEncode($actividad->descripcion->PlaceHolder) ?>" value="<?php echo $actividad->descripcion->EditValue ?>"<?php echo $actividad->descripcion->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($actividad->activa->Visible) { // activa ?>
	<div id="r_activa" class="form-group">
		<label class="<?php echo $actividad_search->SearchLabelClass ?>"><span id="elh_actividad_activa"><?php echo $actividad->activa->FldCaption() ?></span>	
		...
		<div class="<?php echo $actividad_search->SearchRightColumnClass ?>"><div<?php echo $actividad->activa->CellAttributes() ?>>
...
if (is_array($arwrk)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($actividad->activa->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
...
<script type="text/javascript">
factividadsearch.Init();
</script>
<?php
$actividad_search->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$actividad_search->Page_Terminate();
?>
