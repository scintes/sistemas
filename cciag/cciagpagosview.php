...
$pagos_view = NULL; // Initialize page object first

class cpagos_view extends cpagos {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "cciagpagoslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cciagpagoslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cciagpagoslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_pagos\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_pagos',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fpagosview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($pagos_view)) $pagos_view = new cpagos_view();

// Page init
$pagos_view->Page_Init();

// Page main
$pagos_view->Page_Main();
...
// Page object
var pagos_view = new ew_Page("pagos_view");
pagos_view.PageID = "view"; // Page ID
var EW_PAGE_ID = pagos_view.PageID; // For backward compatibility

// Form object
var fpagosview = new ew_Form("fpagosview");

// Form_CustomValidate event
fpagosview.Form_CustomValidate = 
...
fpagosview.ValidateRequired = true;
<?php } else { ?>
fpagosview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpagosview.Lists["x_id_deuda"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_mes","x_anio","x_monto",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpagosview.Lists["x_id_usuario"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_usuario","x_nombre","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if (!$pagos_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($pagos_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
...
?>
<form name="fpagosview" id="fpagosview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="pagos">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_pagosview" class="table table-bordered table-striped">
...
<?php echo $pagos->id_usuario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
