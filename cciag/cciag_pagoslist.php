...
$pagos_list = NULL; // Initialize page object first

class cpagos_list extends cpagos {
	...
	// Page object name
	var $PageObjName = 'pagos_list';

	// Grid form hidden field names
	var $FormName = 'fpagoslist';
	...
	function __construct() {
		...
		$this->AddUrl = "cciag_pagosadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "cciag_pagosdelete.php";
		$this->MultiUpdateUrl = "cciag_pagosupdate.php";
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->id_deuda, $Default, FALSE); // id_deuda
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->id_deuda->AdvancedSearch->Save(); // id_deuda
	...
	function CheckSearchParms() {
		if ($this->id->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->id_deuda->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->id_deuda->AdvancedSearch->UnsetSession();
	...
	function RestoreSearchParms() {
		...
		$this->id_deuda->AdvancedSearch->Load();
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fpagoslist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"cciag_pagossrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fpagoslistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function LoadSearchValues() {
		...
		// id_deuda
		$this->id_deuda->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_id_deuda"]);
		if ($this->id_deuda->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->id_deuda->AdvancedSearch->SearchOperator = @$_GET["z_id_deuda"];
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_pagos\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_pagos',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fpagoslist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
	...
	function ExportQueryString() {
		...
		$this->AddSearchQueryString($sQry, $this->id_deuda); // id_deuda
...
// Create page object
if (!isset($pagos_list)) $pagos_list = new cpagos_list();

// Page init
$pagos_list->Page_Init();

// Page main
$pagos_list->Page_Main();
...
// Page object
var pagos_list = new ew_Page("pagos_list");
pagos_list.PageID = "list"; // Page ID
var EW_PAGE_ID = pagos_list.PageID; // For backward compatibility

// Form object
var fpagoslist = new ew_Form("fpagoslist");
fpagoslist.FormKeyCountName = '<?php echo $pagos_list->FormKeyCountName ?>';

// Form_CustomValidate event
fpagoslist.Form_CustomValidate = 
...
fpagoslist.ValidateRequired = true;
<?php } else { ?>
fpagoslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpagoslist.Lists["x_id_deuda"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_mes","x_anio","x_monto",""],"ParentFields":[],"FilterFields":[],"Options":[]};

// Form object for search
var fpagoslistsrch = new ew_Form("fpagoslistsrch");
</script>
<script type="text/javascript">
...
<?php $pagos_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($pagos->Export == "") { ?>
...
<?php } ?>
<?php if (($pagos->Export == "") || (EW_EXPORT_MASTER_RECORD && $pagos->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "cciag_deudaslist.php";
if ($pagos_list->DbMasterFilter <> "" && $pagos->getCurrentMasterTable() == "deudas") {
...
<?php
	...
	if ($pagos_list->DisplayRecs <= 0 || ($pagos->Export <> "" && $pagos->ExportAll)) // Display all records
		$pagos_list->DisplayRecs = $pagos_list->TotalRecs;
	if (!($pagos->Export <> "" && $pagos->ExportAll))
		$pagos_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$pagos_list->Recordset = $pagos_list->LoadRecordset($pagos_list->StartRec-1, $pagos_list->DisplayRecs);

	// Set no record found message
	if ($pagos->CurrentAction == "" && $pagos_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$pagos_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($pagos_list->SearchWhere == "0=101")
			$pagos_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$pagos_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
...
?>
<?php if ($pagos_list->TotalRecs > 0 || $pagos->CurrentAction <> "") { ?>
<div class="ewGrid">
<?php if ($pagos->Export == "") { ?>
<div class="ewGridUpperPanel">
<?php if ($pagos->CurrentAction <> "gridadd" && $pagos->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($pagos_list->Pager)) $pagos_list->Pager = new cPrevNextPager($pagos_list->StartRec, $pagos_list->DisplayRecs, $pagos_list->TotalRecs) ?>
<?php if ($pagos_list->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($pagos_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $pagos_list->PageUrl() ?>start=<?php echo $pagos_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($pagos_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $pagos_list->PageUrl() ?>start=<?php echo $pagos_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $pagos_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($pagos_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $pagos_list->PageUrl() ?>start=<?php echo $pagos_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($pagos_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $pagos_list->PageUrl() ?>start=<?php echo $pagos_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $pagos_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $pagos_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $pagos_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $pagos_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($pagos_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fpagoslist" id="fpagoslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pagos_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pagos_list->Token ?>">
<?php } ?>
...
<?php if ($pagos_list->TotalRecs > 0) { ?>
<table id="tbl_pagoslist" class="table ewTable">
...
<?php if ($pagos->id->Visible) { // id ?>
	...
	<?php } else { ?>
		<th data-name="id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pagos->SortUrl($pagos->id) ?>',1);"><div id="elh_pagos_id" class="pagos_id">
...
<?php if ($pagos->id_deuda->Visible) { // id_deuda ?>
	...
	<?php } else { ?>
		<th data-name="id_deuda"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pagos->SortUrl($pagos->id_deuda) ?>',1);"><div id="elh_pagos_id_deuda" class="pagos_id_deuda">
...
<?php if ($pagos->fecha->Visible) { // fecha ?>
	...
	<?php } else { ?>
		<th data-name="fecha"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pagos->SortUrl($pagos->fecha) ?>',1);"><div id="elh_pagos_fecha" class="pagos_fecha">
...
<?php if ($pagos->monto->Visible) { // monto ?>
	...
	<?php } else { ?>
		<th data-name="monto"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pagos->SortUrl($pagos->monto) ?>',1);"><div id="elh_pagos_monto" class="pagos_monto">
...
<tbody>
<?php
if ($pagos->ExportAll && $pagos->Export <> "") {
	$pagos_list->StopRec = $pagos_list->TotalRecs;
} else {

	// Set the last record to display
	if ($pagos_list->TotalRecs > $pagos_list->StartRec + $pagos_list->DisplayRecs - 1)
		$pagos_list->StopRec = $pagos_list->StartRec + $pagos_list->DisplayRecs - 1;
	else
		$pagos_list->StopRec = $pagos_list->TotalRecs;
}
$pagos_list->RecCnt = $pagos_list->StartRec - 1;
if ($pagos_list->Recordset && !$pagos_list->Recordset->EOF) {
	$pagos_list->Recordset->MoveFirst();
	$bSelectLimit = EW_SELECT_LIMIT;
	if (!$bSelectLimit && $pagos_list->StartRec > 1)
		$pagos_list->Recordset->Move($pagos_list->StartRec - 1);
} elseif (!$pagos->AllowAddDeleteRow && $pagos_list->StopRec == 0) {
	$pagos_list->StopRec = $pagos->GridAddRowCount;
}
...
while ($pagos_list->RecCnt < $pagos_list->StopRec) {
	$pagos_list->RecCnt++;
	if (intval($pagos_list->RecCnt) >= intval($pagos_list->StartRec)) {
		$pagos_list->RowCnt++;
		...
		$pagos->RowAttrs = array_merge($pagos->RowAttrs, array('data-rowindex'=>$pagos_list->RowCnt, 'id'=>'r' . $pagos_list->RowCnt . '_pagos', 'data-rowtype'=>$pagos->RowType));
...
$pagos_list->ListOptions->Render("body", "left", $pagos_list->RowCnt);
?>
	<?php if ($pagos->id->Visible) { // id ?>
...
<a id="<?php echo $pagos_list->PageObjName . "_row_" . $pagos_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($pagos->id_deuda->Visible) { // id_deuda ?>
...
$pagos_list->ListOptions->Render("body", "right", $pagos_list->RowCnt);
?>
	</tr>
<?php
	}
...
<?php } ?>
<?php if ($pagos_list->TotalRecs == 0 && $pagos->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($pagos_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
