...
$trabajos_foto1_blobview = NULL; // Initialize page object first

class ctrabajos_foto1_blobview extends ctrabajos {
	...
	// Page object name
	var $PageObjName = 'trabajos_foto1_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('trabajos', 'x_foto1');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('foto1');
				...
				if (trim(strval($this->Recordset->fields('foto1'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('foto1') . "\"");
				}
...
// Create page object
if (!isset($trabajos_foto1_blobview)) $trabajos_foto1_blobview = new ctrabajos_foto1_blobview();

// Page init
$trabajos_foto1_blobview->Page_Init();

// Page main
$trabajos_foto1_blobview->Page_Main();
?>
<?php
$trabajos_foto1_blobview->Page_Terminate();
?>
