...
$trabajos_foto2_blobview = NULL; // Initialize page object first

class ctrabajos_foto2_blobview extends ctrabajos {
	...
	// Page object name
	var $PageObjName = 'trabajos_foto2_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('trabajos', 'x_foto2');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('foto2');
				...
				if (trim(strval($this->Recordset->fields('foto2'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('foto2') . "\"");
				}
...
// Create page object
if (!isset($trabajos_foto2_blobview)) $trabajos_foto2_blobview = new ctrabajos_foto2_blobview();

// Page init
$trabajos_foto2_blobview->Page_Init();

// Page main
$trabajos_foto2_blobview->Page_Main();
?>
<?php
$trabajos_foto2_blobview->Page_Terminate();
?>
