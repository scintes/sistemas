...
class cusuario_edit extends cusuario {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// contrasenia
			$this->contrasenia->SetDbValueDef($rsnew, $this->contrasenia->CurrentValue, NULL, $this->contrasenia->ReadOnly);
...
<?php if ($usuario->codigo->Visible) { // codigo ?>
	<tr id="r_codigo">
		<td><span id="elh_usuario_codigo"><?php echo $usuario->codigo->FldCaption() ?></span></td>
		<td<?php echo $usuario->codigo->CellAttributes() ?>>
<span id="el_usuario_codigo" class="control-group">
<span<?php echo $usuario->codigo->ViewAttributes() ?>>
<?php echo $usuario->codigo->EditValue ?></span>
</span>
<input type="hidden" data-field="x_codigo" name="x_codigo" id="x_codigo" value="<?php echo ew_HtmlEncode($usuario->codigo->CurrentValue) ?>">
